            ...
            AndroidView(
                factory = { ctx ->
                    WebView(ctx).apply {
                        ...
                        webViewClient =
                            object : WebViewClient() {
                                override fun onPageFinished(
                                    ...
                                    if (url?.startsWith(Config.YOUTUBE_MUSIC_MAIN_URL) == true) {
                                        coroutineScope.launch {
                                            kotlinx.coroutines.delay(1000) // Wait for cookies to settle
                                            CookieManager.getInstance().flush()
                                            
                                            // Try grabbing cookies from both domains to be safe
                                            val cookie = CookieManager.getInstance().getCookie(url)
                                            val youtubeCookie = CookieManager.getInstance().getCookie("https://youtube.com")
                                            
                                            val finalCookie = if (cookie.orEmpty().length > youtubeCookie.orEmpty().length) cookie else youtubeCookie
                                            ...
                                            if (success) {
                                                ...
                                                navController.navigateUp()
                                                  ...
                                                  // If failed, do NOT clear data immediately so user can try again or we can retry
                                                Toast
                                                    .makeText(
                                                        context,
                        ...
                        settings.userAgentString = "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/130.0.0.0 Mobile Safari/537.36"
