...
object WindowsProtocolRegistrar {
    ...
    fun register() {
        ...
        try {
            ...
            regAdd(REG_KEY, null, "URL:VxMusic Protocol")
    ...
    private fun isAlreadyRegistered(currentExePath: String): Boolean {
        return try {
            ...
            // Registry stores path with quotes: "C:\path\to\VxMusic.exe" "%1"
    ...
    private fun resolveExePath(): String? {
        ...
        //   <app>/VxMusic.exe
        // So we go: java.home → parent (runtime) → parent (app) → VxMusic.exe
        ...
        if (appDir != null) {
            val exeFile = java.io.File(appDir, "VxMusic.exe")
            if (exeFile.exists()) {
