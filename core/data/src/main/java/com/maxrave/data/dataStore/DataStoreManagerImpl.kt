...
import com.maxrave.domain.manager.DataStoreManager.Values.PLAYSTORE
...
internal class DataStoreManagerImpl(
    ...
    override val updateChannel =
        settingsDataStore.data.map { preferences ->
            preferences[UPDATE_CHANNEL] ?: PLAYSTORE
        }
    ...
    override suspend fun setAppLaunchCount(count: Int) {
        withContext(Dispatchers.IO) {
            settingsDataStore.edit { settings ->
                settings[APP_LAUNCH_COUNT] = count
            }
        }
        }
    ...
    override suspend fun setSelectedThemeId(themeId: String) {
        withContext(Dispatchers.IO) {
            settingsDataStore.edit { settings ->
                settings[SELECTED_THEME_ID] = themeId
            }
        }
    }

    override val selectedIconId: Flow<String> =
        settingsDataStore.data.map { preferences ->
            preferences[SELECTED_ICON_ID] ?: "default"
        }

    override suspend fun setSelectedIconId(iconId: String) {
        withContext(Dispatchers.IO) {
            settingsDataStore.edit { settings ->
                settings[SELECTED_ICON_ID] = iconId
            }
        }
    }
    ...
    companion object Settings {
        ...
        val SELECTED_ICON_ID = stringPreferencesKey("selected_icon_id")
    }
}